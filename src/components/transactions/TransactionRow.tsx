...
export function TransactionRow({ t }: { t: Transaction }) {
  const amount = new Intl.NumberFormat(undefined, { style: 'currency', currency: 'USD' }).format(t.amount);
  return (
    <div className={cn('flex items-center justify-between border-b border-border/60 bg-card px-3 py-3 h-full')}> 
      ...
      <div className={cn('text-sm font-semibold', t.amount < 0 ? 'text-danger' : 'text-success')}>{amount}</div>
    </div>
  );
}
