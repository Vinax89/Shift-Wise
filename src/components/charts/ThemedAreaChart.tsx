...
type Props<T extends object> = { data: T[]; xKey: keyof T; yKey: keyof T; height?: number | string; yFormatter?: (n: number) => string };
export default function ThemedAreaChart<T extends Record<string, any>>({ data, xKey, yKey, height = 280, yFormatter }: Props<T>) {
  return (
    <div style={{ height }}>
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ top: 8, right: 12, bottom: 8, left: 8 }}>
          ...
          <Tooltip contentStyle={{ background: 'hsl(var(--chart-bg))', border: '1px solid hsl(var(--border))', color: 'hsl(var(--chart-fg))' }} labelStyle={{ color: 'hsl(var(--chart-fg))' }} cursor={{ stroke: 'hsl(var(--chart-axis-weak))', strokeDasharray: '4 4' }} />
