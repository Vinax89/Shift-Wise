...
// A theme-aware, responsive line chart component built on Recharts.
export default function ThemedLineChart<T extends Record<string, any>>({
  ...
  useEffect(() => {
    setIsClient(true);
    // Colors from CSS vars; Recharts wants CSS color strings, not HSL numbers.
    // This must run client-side after mount to access computed styles.
  ...
  if (!isClient) {
    // Render a skeleton server-side to avoid hydration mismatch,
    // as colors can only be determined on the client.
    return (
      ...
      <div style={{ height }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 8, right: 12, bottom: 8, left: 8 }}>
            ...
            />
            {/* Optional zero-line for legibility */}
            ...
            {yKeys.map((k, i) => {
              ...
              return (
                <Line
                  ...
                  isAnimationActive={false} // respect perf; turn on if you like
                />
              )
            })}
          </LineChart>
